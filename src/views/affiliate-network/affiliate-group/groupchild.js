...
import { ChevronDown,  ChevronLeft,  ChevronRight , Eye ,Delete,Plus , Search} from "react-feather"
...
import {GroupFilterBy } from "../../../configs/affiliateConfig.js"
...
const ActionsComponent = props => {  
  ...
  return (
    <div className="data-list-action">
      <Eye
        className="cursor-pointer mr-1 cursor-pointer"
        size={20}
        onClick={() => {
            ...
            <Col lg='6' md='6' sm='6' xs='6'>
              <FormGroup>
                ...
                <Select
                  ...
                  id="FILTERBY"
                  name="FILTERBY"
                  options={GroupFilterBy}
                  defaultValue={GroupFilterBy[0]}
      ...
      <Row className ={expandClassname}>
        ...
        <Col lg='3' md='3' sm='6' xs='12'>
          <Button
            ...
            outline>
            <Plus size={15} />
            <span className="align-middle" style = {{color : 'white'}}> Create Group </span>
          </Button>
          <Button
            style={{float:'right' , color : 'white'}}
...
class ListViewConfig extends Component {
  ...
  handleFilter = (value,bool) => {
    ...
    if(bool === FILTERBY){
      for(let i = 0 ; i < temp.length ; i ++)
      {
        if(temp[i].bool !== ALL && temp[i].bool !== PERIOD){
          temp[i].bool = value;
  ...
  handleView = (data) => {
    console.log(data);
    this.setState({group_name_input : data.id})
    this.handleSidebar()
  }
        ...
        <Modal isOpen={sidebar} toggle={this.handleSidebar} className="modal-dialog-centered modal-mg">
          ...
          <Form className="" action="#" onSubmit={this.handleregister}>
            <ModalHeader toggle={this.toggleModal} className="bg-primary">
              {this.state.group_name_input ? "UPDATE GROUP" : "ADD NEW GROUP"}
            </ModalHeader>
            <ModalBody className="mt-1">
              <Col>
                <FormGroup>
                  <Label for="username">Group Name</Label>
                  <Input type="text" name="username" id="username" placeholder="Group Name"
                      onChange={e=>this.setState({group_name_input : e.target.value})}
                      value = {this.state.group_name_input}
                      required
                  />
                </FormGroup>
              </Col>
            ...
            <ModalFooter>
              <Row>
                  <Col xs="12 justify-content-start" className = "z-index-0">
                      <Button className="account_submit_btn" color="primary" type="submit">{this.state.group_name_input ? "Update" : "Save"} </Button>
