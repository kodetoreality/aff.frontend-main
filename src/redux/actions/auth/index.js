...
export const get_accessPassword = (privatekey,parameter) =>{
    var str = privatekey;
...
export const setSession = (string)=>{
	sessionStorage.setItem(Root.token,string);
	return true;
}

export const deleteSession = ()=>{
	sessionStorage.setItem(Root.token,undefined);
	return true;
}
...
export const is_session = ()=>{
	if(sessionStorage[Root.token] && sessionStorage[Root.token] !== "undefined"){
		return true;
	}else{
		return false;
	}
...
export const sessioninfor = ()=>{
	return sessionStorage.getItem(Root.token);
}
...
export const md5tostring =(string) =>{
	return get_md5string(string);
}
...
function get_headers (){
	return {'Content-Type': 'application/x-www-form-urlencoded'};// method: 'POST', 'cache-control': 'no-cache', 
}
...
export const AFFILIATE_REQUEST =async (url,inputdata) =>{
	var Response =  await instance.post(Root.sitedomain +"/admin/"+ url,inputdata)
	if(Response.data){
		if(Response.data.session){
			toast.error("Session Destroy");
			deleteSession();
...
export const PLAYERAXIOS_REQUEST =async (url,inputdata) =>{
	var Response =  await instance.post(Root.homepagedomain +"/player/"+ url,inputdata)
	if(Response.data){
		return Response.data;
		// if(Response.data.session){
		// 	toast.error("Session Destroy");
...
export const XPG_XMLREQUEST = (url,parameter,callback) =>{
	request.post(Root.serverurl + url,{ form : parameter, headers: get_headers(), },(err, httpResponse, body)=>{
		if (err) {
			callback({status : false})
		}else{
			var xml = new XMLParser().parseFromString(body);
			var rdata = xml['children'][0]['value']
			if(rdata === "0"){
				callback({status : true,data : xml});
			}else{
				callback({status : false,data :xml['children'][1]['value'] });
			}                     
		}
	});
}

export const get_xpggametype = (type) =>{
	var gametypes = {
		  "1" :"Roulette",
		  "2" :"Blackjack",
		  "4" :"Baccarat",
